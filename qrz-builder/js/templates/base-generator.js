 ...
 *
 * Output is optimized for direct QRZ paste (no manager metadata comments).
 */
...
export function generateFullHtml(sections, theme) {
    ...
    const html = `<!-- QRZ.com Bio Page Content - Copy everything between these comments -->
...
.map-hover-link { border-bottom: 1px dotted rgba(255,255,255,0.35); }
.map-hover-link:hover { border-bottom-color: var(--primary); }
.header-hover-link:hover { color: var(--primary) !important; }
...
.youtube-slide-counter { color:var(--text); font-size:1.1rem; font-weight:600; min-width:80px; text-align:center; }
...
.award-card { background:var(--section-bg); border-radius:15px; box-shadow:0 15px 35px rgba(0,0,0,0.2); overflow:hidden; max-width:85%; width:100%; max-height:90%; display:grid; grid-template-rows:minmax(0,1fr) auto; }
.award-card img { width:100%; height:100%; max-height:none; object-fit:contain; border-bottom:3px solid var(--secondary); background:#000; }
...
.award-info p  { color:var(--text); margin-bottom:0; overflow:visible; }
...
.slide-counter { color:var(--text); font-size:1.1rem; font-weight:600; min-width:80px; text-align:center; }
...
<!-- Footer -->
<footer class="footer">
    <p>Generated by <a href="https://kubabuba.pl/qrz-builder/" target="_blank" rel="noopener noreferrer" style="color:#be954e;text-decoration:none;">QRZ Page Builder</a> &bull; 73 de SP3FCK</p>
</footer>
...
function genHeader(sec, t) {
    ...
    const linksHtml = links.map(l => `
                    <i class="${esc(l.icon || 'fas fa-link')}" style="margin-left: 8px; color: ${l.icon_color ? esc(l.icon_color) : 'var(--secondary)'};"></i>
                    <a href="${esc(l.url)}" target="_blank" rel="noopener noreferrer"
                      class="header-hover-link"
                      style="color:var(--text);text-decoration:none;font-size:1rem;">${esc(l.label)}</a>`).join('');
...
<header class="header">
    <div class="container">
        <div class="header-content">
            <div class="callsign-section">
                ...
                <div class="location">
                    <i class="fas fa-map-marker-alt"></i>${mapsUrl
                        ? `<a href="${esc(mapsUrl)}" target="_blank" rel="noopener noreferrer" class="map-hover-link" style="color:var(--text);text-decoration:none;">${esc(location)}</a>`
                        : esc(location)}
            ...
            ${logoHtml ? `<div style="margin-left:auto;margin-right:15px;">${logoHtml}</div>` : ''}
...
function genTextSection(sec, t) {
    ...
    return `
        ...
        <section class="welcome-section2">
            <h2>${esc(sec.title)}</h2>
            <div class="bio-text">${stripQrzUnsafeChars(d.content || '')}</div>
        </section>`;
}
    ...
    const slidesHtml = slides.length
      ...
      : `
                        <div style="height:100%;display:flex;align-items:center;justify-content:center;">
                            <div class="youtube-card" style="max-width:520px;">
                                <div class="youtube-info" style="padding:40px 30px;">
                                    <h3>No videos yet</h3>
                                    <p>Add at least one YouTube URL to see the carousel in the preview.</p>
                                </div>
                            </div>
                        </div>`;
...
// ── Util ───────────────────────────────────────────────────────────────────────
function esc(str) {
    return stripQrzUnsafeChars(str)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;');
...
function stripQrzUnsafeChars(str) {
    // QRZ may truncate/sanitize biography HTML on some emoji code points.
    // Keep regular Unicode (including Polish diacritics), remove pictographs.
    return String(str ?? '')
